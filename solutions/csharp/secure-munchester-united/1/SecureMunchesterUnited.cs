public class SecurityPassMaker
{
    ...
    {
        string securityPass = support is Staff 
            ? support.Title 
            : "Too Important for a Security Pass";

        return IsPriorityPersonnel(support)
            ? $"{securityPass} Priority Personnel"
            : securityPass;
    }

    private bool IsPriorityPersonnel(TeamSupport support) =>
        support is Security && support is not SecurityJunior
        && support is not SecurityIntern && support is not PoliceLiaison;
}
